[PASTE THE Operations.tsx CONTENT FROM THE FILE BLOCK ABOVE HERE, EXACTLY]import { useState, useRef, useEffect } from 'react';
  ...
  return (
    <div className="operations-container card">
      ...
      <div className="tab-content">
        ...
        {activeTab === 'return' && (
          <ReturnOperation cityKey={cityKey} cityName={cityName} studioKey={studioKey} studioName={studioName} inventory={inventory} studios={studios} onRefresh={onRefresh} />
        )}
        {activeTab === 'laundry' && (
...
function LaundryOperation({ cityKey, cityName, studioKey, studioName, inventory, onRefresh }: OperationComponentProps) {
  ...
  const availableItems = Object.entries(inventory).filter(([_, item]) => {
    if (operation === 'pickup') return item.status === 'In Hamper' && item.studioLocation === studioKey;
